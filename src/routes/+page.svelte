<script>
	 ...
	 */
	function createCheckboxes(language) {
		const punctuationMarks =
			language === EN2CH ? Object.keys(punctuationPairs) : Object.values(punctuationPairs);

		return punctuationMarks.reduce((acc, punctuation) => {
	...
	let text = $state('');
	let inputLanguage = $state(EN2CH);
	let checkboxes = $state(createCheckboxes(EN2CH));
	...
	const keyOrValue = (inputLanguage, dict) => {
		return inputLanguage === EN2CH ? Object.keys(dict) : Object.values(dict);
	};

	function swapLanguage() {
		inputLanguage = inputLanguage === CH2EN ? EN2CH : CH2EN;
		checkboxes = createCheckboxes(inputLanguage);
	}
...
<section aria-labelledby="text-translator-section-heading" class="rounded-inherit mb-0">
	<h1 class="sr-only" id="text-translator-tag-heading">
		{inputLanguage === EN2CH ? '英文标点转中文' : '中文标点转英文'}
	</h1>
			...
			<div class="flex min-h-[56px] w-full flex-row flex-wrap px-3 py-2">
				<div class="flex flex-1 items-center space-x-1.5">
					<div class="flex">
						<div class="mx-1 flex items-center">
							<button
								class="flex items-center bg-gray-100 py-2 px-3 space-x-1 text-base
				...
				<div class="flex justify-between mr-3">
					<div class="flex space-x-2">
						...
						<button
							class="px-4 py-2 text-sm text-white bg-blue-600 rounded hover:bg-blue-700"
							onclick={selectClaude2}
							>Claude2
						</button>
						<button
							class="px-4 py-2 text-sm text-black bg-gray-200 rounded hover:bg-gray-300"
							onclick={deselectAll}
							>清空
						</button>
					</div>
				...
				<div class="flex flex-wrap">
					{#each Object.keys(checkboxes) as punctuation (punctuation)}
						<div class="flex items-center space-x-2 mb-2 mr-4">
							<input
								type="checkbox"
								bind:checked={checkboxes[punctuation]}
								id={punctuation}
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4" id="textareasContainer">
				...
				>
					<div class="flex h-full flex-col">
						...
						<div class="relative flex-1 rounded">
							...
							<button
								class="absolute bottom-2 right-2 px-2 py-1 text-sm text-gray-200 rounded hover:bg-blue-500"
			...
			<div class="relative min-h-[240px] min-w-0 md:min-h-[50vh]">
				<section
					...
					class="h-full flex flex-col rounded border-transparent"
				>
