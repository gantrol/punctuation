...
import { checkItemsAllTrue, checkItemsClaude, MODE_ALL, MODE_CLAUDE } from '$lib/testdata.js';
...
export async function POST({ request }) {
	...
	if (
		typeof text !== 'string' &&
		((mode !== MODE_ALL && mode !== MODE_CLAUDE) || typeof items !== 'object')
	) {
		throw error(
			400,
			'Invalid request format. Ensure `text` is a string, and mode is all or claude, or `checkItems` is an array.'
		);
	}
	...
	if (mode === MODE_ALL) {
		...
		for (const item of /** @type {string[]} */ (items)) {
			customCheckItems[item] = true;
		}
		finalCheckItems = customCheckItems;
	}
